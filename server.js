...
var app = protocol.createServer(function (req, res) {
  ...
  if ((req.method == 'POST') && (pathname == '/postFile.html')) {
    ...
    req.on('end', function () {
      ...
      * str argument is the the string to used
      * @param str argument is the the string to used
      * @return returns the string with the first letter capitalised
      */
      ...
      /**
      * This function is used to insert into tweets into the database
      * @param tweet argument is the JSON of tweets retrieved from the twitter api
      * @return the JSONs are added into the database
      */
      var insertdb = function(tweet){
        // mongo client to use the database
        MongoClient.connect(dan, function (err,db) {
          //if there is an error
          if (err){
            //print to the console error statement
            console.log('Unable to connect to mongodb',err);
          }else {
            //print to the console success statement
      ...
      * The query argument is the the query passed to the database
      * @param the query argument is the the query passed to the database
      * @return the function returns the data needed from the relevant query
      */
      function querydb(q){
          ...
          MongoClient.connect(dan, function (err,db) {
            // if there's an error
            if (err){
              //print to the console the error statement
              console.log('Unable to connect to mongodb',err);
            }else {
              // print to the console the success statment
              console.log('connection established at ',dan);
              // the collection to be used
              var collection = db.collection('twitter');
              // the query to be searched
              var query = ({"entities.user_mentions.screen_name" :q  });
              // find statement
              collection.find(query).toArray(function (err,result) {
                // if there is an error
                if (err){
                  //print to the console the error statment
                  console.log("data was not added",err);
                //else statments that retrie the necessary data needed
                }else {
                  if (result.length == ''){
                     console.log("The query is empty");
                     console.log('queried collection');
                  }else{
                     //print to the console the necessary data
                     for (index in result){
                      x = result[index].user.profile_image_url;
                      console.log("AUTHOR: "+x+result[index].user.name+"   "+"@"+result[index].user.screen_name);
                      console.log("TWEET: "+"  "+result[index].text);
                      console.log("     ");
                     }
                     console.log('queried collection');
      ...
      */
      var resolveData = function(){
        ...
        if(string.query == 'on'&& string.teamname !=''){
          return(querydb(capitalize(string.teamname)));
        }
        if(string.query == 'on'&& string.playername !=''){
          return(querydb(capitalize(string.playername)));
        }
